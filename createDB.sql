...
CREATE TABLE IF NOT EXISTS `musicTracker`.`performance` (
  ...
  PRIMARY KEY (`performanceId`, `venueId`),
...
CREATE TABLE IF NOT EXISTS `musicTracker`.`comment` (
  ...
  `performanceId` INT NULL DEFAULT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `musicTracker`.`favoriteartistinfo` (`username` INT, `artistName` INT, `artistId` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `musicTracker`.`PerformanceSummary` (`performanceId` INT, `title` INT, `duration` INT, `date` INT, `VenuName` INT, `streetAddress` INT, `city` INT, `state` INT, `zipcode` INT, `SongTitle` INT, `Artist` INT, `artistId` INT, `songId` INT, `venueId` INT);
...
CREATE  OR REPLACE VIEW `favoriteartistinfo` AS
    SELECT 
        fav.username, ast.name AS artistName, ast.artistId
    FROM
        favorite fav
            JOIN
        artist ast ON fav.artistId = ast.artistId;
